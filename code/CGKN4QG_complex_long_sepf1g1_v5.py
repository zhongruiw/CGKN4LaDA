...
class GaussianFF(nn.Module):
    def __init__(self, in_dim=2, m=64, sigma=10.0):
        super().__init__()
        self.B = nn.Parameter(torch.randn(m) / sigma)  # learnable if you wish
    def forward(self, x):  # (..., 2)
        x1 = 2*torch.pi * x[..., 0:1] * self.B      # (..., m)
        x2 = 2*torch.pi * x[..., 1:2] * self.B      # (..., m)
        return torch.cat([torch.sin(x1), torch.cos(x1), torch.sin(x2), torch.cos(x2)], dim=-1)
...
class CGN(nn.Module):
    def __init__(self, dim_u1, dim_z, use_pos_encoding=True, num_frequencies=6):
        ...
        if use_pos_encoding:
            self.GaussianFFs = GaussianFF(in_dim=in_dim, m=num_frequencies)
    ...
    def positional_encoding(self, x):
        ...
        embeddings = self.GaussianFFs(x)
        encoded = torch.cat([x, embeddings], dim=1)
        return encoded
...
cgn = CGN(dim_u1, dim_z, use_pos_encoding=True, num_frequencies=16).to(device)
