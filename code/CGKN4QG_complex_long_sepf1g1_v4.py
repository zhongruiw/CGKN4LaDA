...
class StableLinear(nn.Module):
    def __init__(self, d, c=1.0):
        ...
        self.c = c
    def forward(self):
        # A = I + tanh(B)/c     (discrete stable-ish with small c)
        return torch.eye(self.B.shape[0]) + torch.tanh(self.B)/self.c
...
class CGN(nn.Module):
    def __init__(self, dim_u1, dim_z, use_pos_encoding=True, num_frequencies=6):
        ...
        self.g2_B = nn.Parameter(torch.zeros(dim_z, dim_z))
    ...
    def forward(self, x): # x shape(Nt, L, 2)
        ...
        I = torch.eye(self.g2_B.shape[0], device=self.g2_B.device, dtype=self.g2_B.dtype)
        g2_param = I + torch.tanh(self.g2_B) / 1.0
        g2 = g2_param.repeat(Nt, 1, 1)
