...
class CGN(nn.Module):
    def __init__(self, dim_u1, dim_z, use_pos_encoding=True, num_frequencies=6):
        ...
        # U: (.., in_dim) -> (.., 4*rank)
        self.rank = 16
        self.U = nn.Linear(in_dim, 4 * self.rank, bias=False)
        # V: (.., r) -> (.., dim_z)
        self.V = nn.Linear(self.rank, dim_z, bias=False)
    ...
    def forward(self, x): # x shape(Nt, L, 2)
        ...
        # U_out: (Nt, L, 4*r)
        U_out = self.U(x)
        # Reshape to separate the 4 channel-rows per spatial location:
        # (Nt, L, 4, r)
        U_out = U_out.view(Nt, L, 4, self.rank)

        # Apply V on the last dim r -> dim_z.
        # nn.Linear broadcasts over leading dims, so this yields (Nt, L, 4, dim_z)
        g1 = self.V(U_out)

        # Flatten (L,4) -> (L*4)
        g1 = g1.reshape(Nt, L * 4, self.dim_z)

        g1 = nnF.softmax(g1 / 1, dim=-1) # sharp attention 
        # g1 = torch.tanh(g1) * 1   # scale ~ 0.5–2.0
