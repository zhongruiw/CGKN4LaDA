...
class ResBlock(nn.Module):
    def __init__(self, in_ch, out_ch, groups=2, drop_path=0.0, res_scale=1.0):
        super().__init__()
        self.skip_proj = None if in_ch == out_ch else CircularConv2d(in_ch, out_ch, 1, 1, 0, bias=False)
        self.norm1 = nn.GroupNorm(num_groups=groups, num_channels=in_ch)
        self.act1  = nn.SiLU()
        self.conv1 = CircularConv2d(in_ch, out_ch, 3, 1, 1, bias=False)

        self.norm2 = nn.GroupNorm(num_groups=groups, num_channels=out_ch)
        self.act2  = nn.SiLU()
        self.conv2 = CircularConv2d(out_ch, out_ch, 3, 1, 1, bias=True)
        # nn.init.zeros_(self.conv2.weight)  # zero-init second conv
        # if self.conv2.bias is not None: nn.init.zeros_(self.conv2.bias)

        # self.res_scale = res_scale
        # self.drop_path = (lambda x: x) if drop_path == 0 else StochasticDepth(drop_path, "row")
    def forward(self, x):
        h = self.conv1(self.act1(self.norm1(x)))
        h = self.conv2(self.act2(self.norm2(h)))
        # h = self.drop_path(h) * self.res_scale
        skip = x if self.skip_proj is None else self.skip_proj(x)
        return skip + h

# class DoubleConv(nn.Module):
#     def __init__(self, in_channels, out_channels, hidden_channels, groups=8):
#         super().__init__()
#         self.double_conv = nn.Sequential(
#             CircularConv2d(in_channels, hidden_channels, 3, 1, 1, bias=False),
#             nn.GroupNorm(num_groups=groups, num_channels=hidden_channels),
#             nn.SiLU(),
#             CircularConv2d(hidden_channels, out_channels, 3, 1, 1, bias=False),
...
# class ResBlock(nn.Module):
#     def __init__(self, in_channels, hidden_channels, groups=8):
#         super().__init__()
#         self.double_conv = DoubleConv(in_channels, in_channels, hidden_channels, groups)
#         # ReZero-style scaling to tame early training
#         self.alpha = nn.Parameter(torch.zeros(1))

#     def forward(self, x):
#         return x + self.alpha * self.double_conv(x)
...
class UpBlock(nn.Module):
    def __init__(self, in_channels, out_channels, mode="bilinear"):
        ...
        # self.up = nn.Sequential(
        #     nn.Upsample(scale_factor=2, mode=mode, align_corners=False if mode == "bilinear" else None),
        #     CircularConv2d(in_channels, out_channels, 3, 1, 1),
        #     nn.SiLU(),
        #     )
...
class Encoder(nn.Module):
    ...
    def __init__(self, in_channels=2, out_channels=2, hidden_channels=16, depth=2):
        super().__init__()
        self.pre = ResBlock(in_channels, hidden_channels)
...
class Decoder(nn.Module):
    ...
    def __init__(self, in_channels=2, out_channels=2, hidden_channels=16, depth=2):
        ...
        self.post = ResBlock(hidden_channels, out_channels)
