...
class LagrangianOI:
    ...
    def analyze(
        ...
        xy_obs_prev: np.ndarray,      # observed positions at t0, shape (L,2) in [0,2π)
        xy_obs_now: np.ndarray        # observed positions at t0+Δt_obs, shape (L,2)
    ):
        """
        ...
        psi1_k_ens, x_traj, y_traj, _ = self.model.forward_ens(
            ...
            qp_ens=qp0_ens, L=L, x0=x0, y0=y0
        )
        xy_b_now = np.stack([x_traj[0, :, -1], y_traj[0, :, -1]], axis=1)
        vb = self._positions_to_vel(xy_b_now, xy_obs_prev)     # (L,2)

        # ------------- observed Lagrangian vel from two positions --------------
        vo = self._positions_to_vel(xy_obs_now, xy_obs_prev)   # (L,2)
...
for isig, sigma_b_vel in enumerate(sigma_b_vels):
    ...
    for iloc, localization_radius in enumerate(localization_radiuses):
        ...
        for iassim in range(1, nobstime):
            # if iassim % 100 == 0:
            #     print('iassim:', iassim)
            #     mse_prior = np.mean((psi_b - psi_truth[iassim-1])**2)
            #     print('prior mse:', mse_prior)
                
            xy_prev = xy_obs[iassim-1, :L, :]   # r^o(t0)
            xy_now  = xy_obs[iassim,   :L, :]   # r^o(t0+Δt)
            ...
            # if iassim % 100 == 0:
            #     mse_analy = np.mean((psi_b - psi_truth[iassim-1])**2)
            #     print('analy mse:', mse_analy)
