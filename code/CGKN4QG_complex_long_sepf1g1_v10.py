...
class Encoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.enc = nn.Sequential(
            CircularConv2d(in_channels, 32, kernel_size=3, stride=1, padding=1), nn.SiLU(),
            ...
            CircularConv2d(32, out_channels, kernel_size=3, stride=1, padding=1)
        )
...
class Decoder(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.dec = nn.Sequential(
            CircularConvTranspose2d(in_channels, 32, kernel_size=3, stride=1, padding=1), nn.SiLU(),
            ...
            CircularConvTranspose2d(32, out_channels, kernel_size=3, stride=1, padding=1)
        )
...
class CGN(nn.Module):
    def __init__(self, dim_u1, dim_z, use_pos_encoding=True, num_frequencies=6):
        ...
        self.g2_delta = nn.Parameter(torch.randn(dim_z, dim_z) / dim_z)
    ...
    def forward(self, x): # x shape(Nt, L, 2)
        ...
        f2 = self.f2_param.unsqueeze(0).expand(Nt, -1, -1)
        g2 = torch.eye(dim_z, device=x.device) + self.g2_delta
        g2 = g2.unsqueeze(0).expand(Nt, -1, -1)
...
autoencoder = AutoEncoder(in_channels=2, latent_channels=latent_channels).to(device)
cgn = CGN(dim_u1, dim_z, use_pos_encoding=True, num_frequencies=6).to(device)
cgkn = CGKN(autoencoder, cgn).to(device)
optimizer = torch.optim.AdamW(cgkn.parameters(), lr=1e-3, weight_decay=1e-4)
...
for ep in range(1, epochs+1):
    ...
    for u1_initial, u2_initial, u1_next, u2_next in train_loader:
        ...
        z_cycle = cgkn.autoencoder.encoder(u2_initial_ae)
        loss_ae_cyc = nnF.mse_loss(z_initial, z_cycle)
        ...
        loss_total = loss_forecast_u1 + loss_forecast_u2 + loss_ae + loss_forecast_z + loss_ae_cyc
        ...
        train_loss_ae_cyc += loss_ae_cyc.item()
    ...
    train_loss_ae_cyc /= train_num_batches
    ...
    if ep % 10 == 0:
        ...
        if val_loss_total < best_val_loss:
            ...
            # torch.save(checkpoint, r"../model/CGKN_L1024_complex_long_sepf1g1_v3_stage1.pt")
            status = "✅"
        else:
            status = ""
        ...
        print(f"ep {ep} time {end_time - start_time:.4f} | "
              f"train_u1: {train_loss_forecast_u1:.4f}  train_u2: {train_loss_forecast_u2:.4f} "
              f"train_z: {train_loss_forecast_z:.4f}  ae: {train_loss_ae:.4f} ae_cyc: {train_loss_ae_cyc:.4f}"
              )

# np.savez(r"../model/CGKN_L1024_complex_long_sepf1g1_v3_loss_history.npz", **loss_history)
# """
...
# sigma_hat = torch.load("../data/CGKN_L1024_complex_long_sepf1g1_v3_sigma_hat.pt", weights_only=True)
